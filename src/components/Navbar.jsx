    ...
    <>
      ...
      >
        <div className="flex justify-between items-center px-8 py-4 max-w-7xl mx-auto">
          {/* Logo */}
          <div className="text-2xl text-[#FFFFFF]" style={{ fontFamily: "'Arial Rounded MT Bold', 'Varela Round', sans-serif", letterSpacing: "0.5px" }}>
            Portfolio
          </div>
          ...
          <div className="hidden md:flex gap-8 items-center">
            {navLinks.map((link) => (
              <a
                ...
                className={
                  link.name === "Contact"
                    ? "font-headline tracking-tighter uppercase text-sm px-5 py-2 rounded-full bg-[#fff0f0ff] text-[#0a0a0a] border border-white/70 shadow-[0_0_18px_rgba(255,255,255,0.22)] transition-all duration-200 hover:scale-[1.03] hover:bg-white"
                    : "font-headline tracking-tighter uppercase text-sm text-[#FFFFFF] hover:text-[#00ff9f] transition-all duration-200 hover:[text-shadow:_0_0_8px_#00ff9f]"
                }
                href={link.href}
              >
                {link.name}
          ...
          <div
            className={`absolute top-16 right-4 w-48 bg-[#0a0a0a]/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden transition-all duration-300 ease-out p-2 z-50 ${isOpen
            ...
            <div className="flex flex-col gap-1">
              {navLinks.map((link, i) => (
                <a
                  ...
                  href={link.href}
                  onClick={() => setIsOpen(false)}
                  className="px-4 py-3 text-[#FFFFFF] hover:text-[#00ff9f] hover:bg-white/5 rounded-xl transition-all font-headline tracking-tighter uppercase text-[11px] flex items-center justify-between group"
